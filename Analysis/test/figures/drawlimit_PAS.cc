...
void drawlimit_PAS(TString isZA) {
  ...
  auto ytext = std::make_unique<TPaveText>(0.0,0.12,0.4,0.92,"NDC");
  ytext->SetBorderSize(0);
  ytext->SetFillColor(0);
  ytext->SetFillStyle(0);
  TString Ychar = isZA=="A" ? "Y" : "Z";
  TString astr = "\\sigma(\\mathrm{pp}\\rightarrow\\mathrm{X}) \\times \\mathrm{B}(\\mathrm{X}\\rightarrow\\mathrm{"+Ychar+"Y}\\rightarrow\\!4\\ell) [\\mathrm{fb}]";
  ytext->AddText(astr);
  ...
  auto ytext2 = std::make_unique<TPaveText>(0.0,0.25,0.4,0.4,"NDC");
  ...
  SaveAs(canvas1,std::string((TString("limit1D_")+isZA+"_aux_"+era+".png").Data()));
